...
{
	if (m_pRoot == NULL || ref.Key == (m_pRoot->Ref).Key)
	{
		std::cout << "Head ptr" << std::endl;
		return NULL;
	}
	...
	while (pNode != NULL)
	{
		...
		//only compare if left child is not Null
		if (pNode -> pLeft != NULL)
		{
			if (((pNode -> pLeft) -> Ref).Key == ref.Key)
			{
				return pNode;
			}
		...
		//only compare if right child is not Null
		if (pNode -> pRight != NULL)
		{
			if (((pNode -> pRight) -> Ref).Key == ref.Key)
			{
				return pNode;
			}
...
void LeftLeaningRedBlack::LeafDepth(void)
{
	...
	if (NULL != m_pRoot) {
		maxDepth = LeafDepthRec(m_pRoot, 0, ary, arraysize, 0, minBlack, maxBlack);
	}
